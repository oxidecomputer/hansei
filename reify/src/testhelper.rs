//! Synthetic bundles for reify's tests.
//!
//! These are hand-built `Bundle`s rather than extracted ones: they pin the
//! exact type graph a test needs, so a render assertion cannot drift because
//! an extractor changed what it emits. Two independent fixtures live here so
//! neither one's layout can perturb the other's tests.
